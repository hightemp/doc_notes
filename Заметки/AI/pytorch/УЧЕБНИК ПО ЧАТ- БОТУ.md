https://pytorch.org/tutorials/beginner/chatbot_tutorial.html?highlight=chat%20bot
...
-   Обрабатывать загрузку и предварительную обработку набора данных [Cornell Movie-Dialogs Corpus.](https://www.cs.cornell.edu/~cristian/Cornell_Movie-Dialogs_Corpus.html)
...
-   `loadLinesAndConversations`разбивает каждую строку файла на словарь строк с полями: lineID, characterID и текст, а затем группирует их в диалоги с полями: talkID, movieID и строки.
...
Теперь мы вызовем эти функции и создадим файл. Мы назовем его _formatted_movie_lines.txt_ .
...
## Подготовьте данные для моделей[](https://pytorch.org/tutorials/beginner/chatbot_tutorial.html?highlight=chat%20bot#prepare-data-for-models)
...
print("input_variable:", [input_variable](https://pytorch.org/docs/stable/tensors.html#torch.Tensor "torch.Tensor"))
...
### Одна итерация обучения[](https://pytorch.org/tutorials/beginner/chatbot_tutorial.html?highlight=chat%20bot#single-training-iteration)
...
## Определить оценку[](https://pytorch.org/tutorials/beginner/chatbot_tutorial.html?highlight=chat%20bot#define-evaluation)
...
### Оцените мой текст[](https://pytorch.org/tutorials/beginner/chatbot_tutorial.html?highlight=chat%20bot#evaluate-my-text)
...
## Запустить модель[](https://pytorch.org/tutorials/beginner/chatbot_tutorial.html?highlight=chat%20bot#run-model)

Наконец, пришло время запустить нашу модель!

Независимо от того, хотим ли мы обучать или тестировать модель чат- бота , мы должны инициализировать отдельные модели кодировщика и декодера. В следующем блоке мы устанавливаем желаемые конфигурации, выбираем запуск с нуля или устанавливаем контрольную точку для загрузки, а также строим и инициализируем модели. Не стесняйтесь экспериментировать с различными конфигурациями моделей для оптимизации производительности.
...
if loadFilename:
    ...
    checkpoint = [torch.load](https://pytorch.org/docs/stable/generated/torch.load.html#torch.load "torch.load")(loadFilename)
...
[embedding](https://pytorch.org/docs/stable/generated/torch.nn.Embedding.html#torch.nn.Embedding "torch.nn.Embedding") = [nn.Embedding](https://pytorch.org/docs/stable/generated/torch.nn.Embedding.html#torch.nn.Embedding "torch.nn.Embedding")(voc.num_words, hidden_size)
if loadFilename:
    [embedding.load_state_dict](https://pytorch.org/docs/stable/generated/torch.nn.Module.html#torch.nn.Module.load_state_dict "torch.nn.Module.load_state_dict")(embedding_sd)
# Initialize encoder & decoder models
encoder = [EncoderRNN](https://pytorch.org/docs/stable/generated/torch.nn.Module.html#torch.nn.Module "torch.nn.Module")(hidden_size, [embedding](https://pytorch.org/docs/stable/generated/torch.nn.Embedding.html#torch.nn.Embedding "torch.nn.Embedding"), encoder_n_layers, dropout)
decoder = [LuongAttnDecoderRNN](https://pytorch.org/docs/stable/generated/torch.nn.Module.html#torch.nn.Module "torch.nn.Module")(attn_model, [embedding](https://pytorch.org/docs/stable/generated/torch.nn.Embedding.html#torch.nn.Embedding "torch.nn.Embedding"), hidden_size, voc.num_words, decoder_n_layers, dropout)
if loadFilename:
    [encoder.load_state_dict](https://pytorch.org/docs/stable/generated/torch.nn.Module.html#torch.nn.Module.load_state_dict "torch.nn.Module.load_state_dict")(encoder_sd)
    [decoder.load_state_dict](https://pytorch.org/docs/stable/generated/torch.nn.Module.html#torch.nn.Module.load_state_dict "torch.nn.Module.load_state_dict")(decoder_sd)
# Use appropriate device
encoder = [encoder.to](https://pytorch.org/docs/stable/generated/torch.nn.Module.html#torch.nn.Module.to "torch.nn.Module.to")([device](https://pytorch.org/docs/stable/tensor_attributes.html#torch.device "torch.device"))
decoder = [decoder.to](https://pytorch.org/docs/stable/generated/torch.nn.Module.html#torch.nn.Module.to "torch.nn.Module.to")([device](https://pytorch.org/docs/stable/tensor_attributes.html#torch.device "torch.device"))
print('Models built and ready to go!')

Building encoder and decoder ...
Models built and ready to go!

### Выполнить обучение[](https://pytorch.org/tutorials/beginner/chatbot_tutorial.html?highlight=chat%20bot#run-training)

Запустите следующий блок, если вы хотите обучить модель.

Сначала мы устанавливаем параметры обучения, затем инициализируем наши оптимизаторы и, наконец, вызываем функцию `trainIters`для запуска наших итераций обучения.
...
[encoder.train](https://pytorch.org/docs/stable/generated/torch.nn.Module.html#torch.nn.Module.train "torch.nn.Module.train")()
[decoder.train](https://pytorch.org/docs/stable/generated/torch.nn.Module.html#torch.nn.Module.train "torch.nn.Module.train")()

# Initialize optimizers
print('Building optimizers ...')
[encoder_optimizer](https://pytorch.org/docs/stable/generated/torch.optim.Adam.html#torch.optim.Adam "torch.optim.Adam") = [optim.Adam](https://pytorch.org/docs/stable/generated/torch.optim.Adam.html#torch.optim.Adam "torch.optim.Adam")([encoder.parameters](https://pytorch.org/docs/stable/generated/torch.nn.Module.html#torch.nn.Module.parameters "torch.nn.Module.parameters")(), lr=learning_rate)
[decoder_optimizer](https://pytorch.org/docs/stable/generated/torch.optim.Adam.html#torch.optim.Adam "torch.optim.Adam") = [optim.Adam](https://pytorch.org/docs/stable/generated/torch.optim.Adam.html#torch.optim.Adam "torch.optim.Adam")([decoder.parameters](https://pytorch.org/docs/stable/generated/torch.nn.Module.html#torch.nn.Module.parameters "torch.nn.Module.parameters")(), lr=learning_rate * decoder_learning_ratio)
if loadFilename:
    [encoder_optimizer.load_state_dict](https://pytorch.org/docs/stable/generated/torch.optim.Adam.html#torch.optim.Adam.load_state_dict "torch.optim.Adam.load_state_dict")(encoder_optimizer_sd)
    [decoder_optimizer.load_state_dict](https://pytorch.org/docs/stable/generated/torch.optim.Adam.html#torch.optim.Adam.load_state_dict "torch.optim.Adam.load_state_dict")(decoder_optimizer_sd)

# If you have cuda, configure cuda to call
for state in [encoder_optimizer](https://pytorch.org/docs/stable/generated/torch.optim.Adam.html#torch.optim.Adam "torch.optim.Adam").state.values():
    for k, v in state.items():
        if isinstance(v, [torch.Tensor](https://pytorch.org/docs/stable/tensors.html#torch.Tensor "torch.Tensor")):
            state[k] = v.cuda()

for state in [decoder_optimizer](https://pytorch.org/docs/stable/generated/torch.optim.Adam.html#torch.optim.Adam "torch.optim.Adam").state.values():
    for k, v in state.items():
        if isinstance(v, [torch.Tensor](https://pytorch.org/docs/stable/tensors.html#torch.Tensor "torch.Tensor")):
            state[k] = v.cuda()
...
trainIters(model_name, voc, pairs, encoder, decoder, [encoder_optimizer](https://pytorch.org/docs/stable/generated/torch.optim.Adam.html#torch.optim.Adam "torch.optim.Adam"), [decoder_optimizer](https://pytorch.org/docs/stable/generated/torch.optim.Adam.html#torch.optim.Adam "torch.optim.Adam"),
           [embedding](https://pytorch.org/docs/stable/generated/torch.nn.Embedding.html#torch.nn.Embedding "torch.nn.Embedding"), encoder_n_layers, decoder_n_layers, save_dir, n_iteration, batch_size,
...
Iteration: 4000; Percent complete: 100.0%; Average loss: 2.5053

### Запустить оценку[](https://pytorch.org/tutorials/beginner/chatbot_tutorial.html?highlight=chat%20bot#run-evaluation)
...
searcher = [GreedySearchDecoder](https://pytorch.org/docs/stable/generated/torch.nn.Module.html#torch.nn.Module "torch.nn.Module")(encoder, decoder)

# Begin chatting (uncomment and run the following line to begin)
# evaluateInput(encoder, decoder, searcher, voc)

## Заключение[](https://pytorch.org/tutorials/beginner/chatbot_tutorial.html?highlight=chat%20bot#conclusion)

На этом все, ребята. Поздравляем, теперь вы знаете основы создания модели генеративного чат- бота ! Если вам интересно, вы можете попробовать настроить поведение чат-бота, изменив модель и параметры обучения, а также настроив данные, на которых вы обучаете модель.

Ознакомьтесь с другими руководствами, чтобы узнать о других интересных приложениях для глубокого обучения в PyTorch!

**Общее время работы скрипта:** ( 5 минут 46.009 секунд)